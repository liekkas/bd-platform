...
class GroupUserAnalysis extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      kpi: kpis[0],
  ...
  _getData(bind, props, dateType = 'D', start = '20150501', end = '20151031') {
    fetch(REST_API_BASE_URL + 'groupUserAnalysis?dateType=' + dateType + '&start=' + start + '&end=' + end)
  ...
  search(dateType,start,end) {
    console.log('>>> GroupUserAnalysis Search:',dateType,start,end)
  ...
  render() {
    ...
    return (
      <div className={style.root}>
        ...
        <DataGrid title="频道组用户分析" columns={columns} datas={this.state.tableData}/>
      </div>
    )
  }
}

GroupUserAnalysis.propTypes = {
}

export default GroupUserAnalysis
