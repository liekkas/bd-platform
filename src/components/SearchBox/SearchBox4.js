...
class SearchBox4 extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
    ...
    return (
      <div className={style.root}>
        ...
        <Select defaultValue={this.props.channelType}
          ...
          <Option value="3">具体频道</Option>
        </Select>

        &nbsp;&nbsp;

        {
          this.state.channelType === '3'
            ? <ByChannel onChannelChange={(v) => this.setState({channelName:v})}/>
            : null
        }
        &nbsp;&nbsp;

        <Button type="primary" onClick={() =>
          this.props.onSearch(this.state.channelType,this.state.channelName,this.state.dateType,this.state.start)}>
          <Icon type="search" />
          查询
        </Button>
